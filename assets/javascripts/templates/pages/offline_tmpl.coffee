app.templates.offlinePage = (docs) -> """
  ...
  <dl>
    ...
    <dd>Each page is cached as a key-value pair in <a href="https://docs.ersolucoesweb.com.br/dom/indexeddb_api">IndexedDB</a> (downloaded from a single file).<br>
        The app also uses <a href="https://docs.ersolucoesweb.com.br/dom/service_worker_api/using_service_workers">Service Workers</a> and <a href="https://docs.ersolucoesweb.com.br/dom/web_storage_api">localStorage</a> to cache the assets and index files.
...
canICloseTheTab = ->
  if app.ServiceWorker.isEnabled()
    """ Yes! Even offline, you can open a new tab, go to <a href="//docs.ersolucoesweb.com.br">docs.ersolucoesweb.com.br</a>, and everything will work as if you were online (provided you installed all the documentations you want to use beforehand). """
  else
    ...
    """ No. Service Workers #{reason}, so loading <a href="//docs.ersolucoesweb.com.br">docs.ersolucoesweb.com.br</a> offline won't work.<br>
